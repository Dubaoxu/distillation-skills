...
Shared autonomy [29,30] recognizes that in human-robot interaction, both the
...
human intent with robotic assistance. In the context of assistive navigation,
shared autonomy addresses a key ethical and practical concern: the user must
retain agency over their own movement. Our framework instantiates shared
autonomy by having the RL layer infer user intent (expressed as cost function
parameters) while the Tube MPC layer enforces safety—the user's preferences
determine *what* is optimized, and the system guarantees *how* it is executed.
